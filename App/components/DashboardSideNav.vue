                ...
                <div class="flex flex-col flex-1 justify-between">
                    <div class="pt-5 pb-4 overflow-y-auto">
                        ...
                        <nav class="mt-5 flex-1 px-2 bg-gray-800 space-y-1">
                            <nuxt-link :to="item.path" v-for="(item, index) in menuItems" :key="index" class="text-gray-300 hover:bg-gray-700 hover:text-white group flex items-center px-2 py-2 text-base font-medium rounded-md">
                                <fa :icon="item.icon" class="mr-2" style="width: 2rem;" />
                                <span>{{ item.name }}</span>
                            </nuxt-link>
                        </nav>
                        <div class="block border-b border-gray-900 my-5"></div>
                        <button @click="toggleListAccounts" class="flex items-center justify-between focus:outline-none w-full px-6">
                            <div class="flex items-center">
                                <fa :icon="(listAccounts) ? 'chevron-down' : 'chevron-right'" class="text-xs text-gray-500 mr-1" style="width: 1rem;" />
                                <span class="text-gray-300 text-xs font-medium uppercase tracking-wide">Accounts</span>
                            </div>
                            <span class="text-green-500 font-bold">$1,000</span>
                        </button>
                        <div v-show="listAccounts" class="px-6 mt-2">
                            <p class="text-gray-300">List accounts and values...</p>
                        </div>
                    </div>
                    <div class="flex-shrink-0 flex bg-gray-700 p-4">
                        <a href="#" class="flex-shrink-0 w-full group block">
                            <div class="flex items-center">
                                <div>
                                    <img class="inline-block h-9 w-9 rounded-full" src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&ixqx=aueEilRdQ2&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80" alt="">
                                </div>
                                <div class="ml-3">
...
export default {
    data () {
        return {
            menuItems: [
                { icon: 'dollar-sign', name: 'Dashboard', path: '/dashboard' },
                { icon: 'stream', name: 'Accounts', path: '/accounts' },
                { icon: 'chart-pie', name: 'Reports', path: '/reports' },
            ],
            listAccounts: true,
            mobileNavOpen: false
        }
